...
def callback(data):
		global a
		global b
		dtd_mine  = rospy.Publisher('/HRATC_FW/set_mine', PoseStamped, queue_size=1)
		...
		if (leftCoil==1):
			try:			
				...
				(trans1,rot1) = listener.lookupTransform('/minefield', '/left_coil', now)
				x1=trans1[0]   
				y1=trans1[1]    
				z1=trans1[2]    
				c=abs(x1)-abs(a)
		...
		if (rightCoil==1):
			...
			try:			
				...
				(trans2,rot2) = listener.lookupTransform('/minefield', '/right_coil', now)
				x2=trans2[0]   
				y2=trans2[1]    
				z2=trans2[2]    
				c=abs(x2)-abs(a)
		...
		if (leftCoil==1 and rightCoil==1):
			try:			
				...
				(trans3,rot3) = listener.lookupTransform('/minefield', '/right_coil', now)
				(trans4,rot4) = listener.lookupTransform('/minefield', '/left_coil', now)
				x3=trans3[0]   
				y3=trans3[1]    
				z3=trans3[2]
...
def sub():
	...
	rospy.Subscriber("/coils",Coil,callback)
